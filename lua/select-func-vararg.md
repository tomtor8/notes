...
local function count\_args(...)  
    \-- Select '\#' returns the exact number of passed parameters  
    local num \= select('\#', ...)  
    print("Argument count:", num)  
end

count\_args("a", "b", "c") \-- Output: Argument count: 3  
count\_args("a", nil, "c") \-- Output: Argument count: 3 (correctly counts nil)
...
local function print\_sub\_args(...)
...
print\_sub\_args("apple", "banana", "cherry", "date")
...
local function demonstrate\_pack(...)  
...
demonstrate\_pack(10, nil, 30, nil)
...
\-- Less efficient: Creates a table every time sum\_numbers\_pack is called

local function sum\_numbers\_pack(...)  
...
2. **Inside expressions:** Passing `select(i, ...)` as an argument to another function (unless it is the *last* argument) or wrapping it in parentheses `(select(i, ...))` also truncates the result to a single value.
...
local function demo(...)
    ...
    local first\_only \= select(2, ...)

    print("Assigned to 1 var:", first\_only) \-- Output: "banana"
...
In your loop snippet, writing `local item = select(i, ...)` was deliberately using this truncation behavior as a trick to pick *only* the i-th element out of the tuple\!
...
- **Not the Last Position in a Call:** `print(..., "end")` → Evaluates only the *first* value of `...`, followed by `"end"`. All trailing vararg items are dropped.  
- **Not the Last Item in Assignments:** `local a, b = ..., 10` → Sets `a` to the *first* value of `...`, and `b = 10`.  
...
local function show\_rules(...)
    ...
    local full\_table \= { "a", ... }   \-- {"a", 10, 20, 30}

    local trimmed\_table \= { ..., "z" } \-- {10, "z"}

end

show\_rules(10, 20, 30\)
