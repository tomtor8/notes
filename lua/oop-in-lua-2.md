*Grammar note: The compound adjective "easy-to-follow" or "straightforward" fits best when introducing concise patterns.*

### **1\. Private Fields (Data Hiding using Closures)**
...
local function BankAccount(initial\_balance)  
    \-- Private variable (hidden inside the closure)  
    local balance \= initial\_balance or 0
...
### **2\. Multiple Inheritance (Using Metatable \_\_index Function)**

Since Lua tables only have one metatable, multiple inheritance is achieved by setting \_\_index to a **custom function** that searches through a list of parent classes sequentially.  
\-- Helper function to search parents for a key  
local function search\_parents(key, parents)  
...
function Duck:new(name)  
    ...
    setmetatable(instance, {  
        \_\_index \= function(tbl, key)  
            return search\_parents(key, parents)  
        end  
    })  
...
> * **Multiple Inheritance:** Use a custom function for \_\_index that iterates through an array/list of parent tables to locate methods dynamically.