...
local add_mt = {
    ...
    __le = function(x,y) -- less than or equal, not for 2 strings
        return string.len(x) <= y
    end,
}

debug.setmetatable(str1, add_mt)
...
The `__le` works with combinations of e.g. strings and numbers.  
...
**Outputs:** Tom _ Tom _ Tom _ Tom _ Tom
