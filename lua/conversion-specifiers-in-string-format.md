# Conversion specifiers in string.format()
...
- **%q** _(Lua-specific)_: Formats a string safely enclosed in double quotes with escape sequences so it can be safely read back by the Lua interpreter (e.g., handles newlines, quotes, and control characters).
...
| _(space)_ | Inserts a single space before positive numbers if no sign is written.                      | string.format("% d", 42)     | " 42"  |
...
local code = string.format("local path = %q", "C:Luascript.luan")
-- Output: local path = "C:Luascript.luan"
```
