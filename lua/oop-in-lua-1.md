...
Lua doesn't have a built-in class keyword. Instead, OOP is implemented using tables as prototypes and setting `__index` to point back to the class table.

## Basic Class & Methods

A class is simply a table containing methods. When an instance is created, its metatable points `__index` to the class table, allowing instances to inherit those methods.

```lua
-- Define the Class table
local Vector = {}
Vector.__index = Vector -- Look up missing keys in 'Vector'
...
To create a subclass, set the subclass's metatable so its `__index` points to the parent class.
...
2. Lua checks Dog via `__index`. Found and executed.
3. If Dog didn't define speak, Lua checks Animal via Dog's metatable `__index`.
...
**Step 2: Setting up Prototype Lookup (`Vector.__index = Vector`)**

- **What happens:** The key `__index` inside the Vector table is assigned to point back to the Vector table itself.
- **Under the hood:** When a table uses Vector as its metatable, Lua checks the `__index` key whenever a field or method isn't found in that table. Pointing `Vector.__index = Vector` ensures that missing keys on instances resolve directly to Vector's methods.
...
- **Under the hood search:**
  ...
  4. It inspects Vector's `__index` field, which points to Vector itself.
...
- **Under the hood:**
  ...
  - Because `Animal:new()` runs `setmetatable(instance, self)` and `self.__index = self` (where self is Animal), Dog inherits from Animal.
...
It is completely valid to use `setmetatable(instance, self)` in `Vector:new(x, y)`.
